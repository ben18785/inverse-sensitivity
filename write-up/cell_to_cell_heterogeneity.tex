...
\end{equation}
%
The left and right hand sides of eq. (\ref{eq:joint}) can be rearranged to obtain a form for the posterior input distribution,
%
\begin{equation}
...
\end{equation}
%
Given parameters $\boldsymbol{\theta}$, the mapping from parameters to outputs is deterministic meaning $p(\tilde{\boldsymbol{x}}| \boldsymbol{\theta}, \hat{\phi})=\delta(\tilde{\boldsymbol{x}}(\boldsymbol{\theta}))$. In what follows, we assume that the conditional distribution $p(\boldsymbol{\theta}|\tilde{\boldsymbol{x}}, \hat{\phi})$ is independent of the data, meaning it represents a conditional ``prior'', which can be manipulated by Bayes' rule,
%
\begin{equation}
p(\boldsymbol{\theta}|\tilde{\boldsymbol{x}}(\boldsymbol{\theta})) = \frac{p(\boldsymbol{\theta})}{p(\tilde{\boldsymbol{x}}(\boldsymbol{\theta}))},
\end{equation}
%
...
p(\boldsymbol{\theta}|\hat{\phi}) = \frac{p(\boldsymbol{\theta})}{p(\tilde{\boldsymbol{x}}(\boldsymbol{\theta}))} p(\tilde{\boldsymbol{x}}(\boldsymbol{\theta})|\hat{\phi}).
\end{equation}
%
Again, we refer to our companion piece [citation] for detailed explanation of eq. (\ref{eq:posterior_input}) and instead here provide brief interpretation when considering a uniform prior on parameter space. In this case, $p(\boldsymbol{\theta}) = \frac{1}{V}$, where $V$ is the total volume of parameter space. 
